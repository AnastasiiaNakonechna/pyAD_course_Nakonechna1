...
def index(request):
    teachers = Teachers.objects.order_by('?').last()
    context = {
        'name': 'Anastasiia',
        'teachers': teachers,
    }
...
def create_teachers(request):
    from studentsHW.forms import TeachersCreateForm

    form = TeachersCreateForm(request.GET)
...
def index1(request):
    group = Group.objects.order_by('?').last()
    context = {
        'name': 'Anastasiia',
        'teachers': group,
    }
...
def create_group(request):
    from studentsHW.forms import GroupCreateForm
