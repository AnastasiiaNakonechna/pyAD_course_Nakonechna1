...
def students_group(request):
    ...
    f = ''
    for _ in range(0, 31):
        students = Group.objects.create(first_name=fake.first_name(), last_name=fake.last_name(),
                                         scores_in_group=int(random.uniform(17, 51)))
...
def teachers_list(request):
    ...
    f = ''
    for _ in range(0, 16):
        teachers = Teachers.objects.create(first_name=fake.first_name(), last_name=fake.last_name())
        f += f'N{teachers.id} Teacher: first_name: {teachers.first_name}; last_name: {teachers.last_name}<br/>'
        # print(f)
